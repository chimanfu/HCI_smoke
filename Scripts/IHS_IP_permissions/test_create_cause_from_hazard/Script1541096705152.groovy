...
for (row = 1; row <= findTestData(ip_test_user_list).getRowNumbers(); row++){
	...
	create_record_from_hazard_casue_link(product,component,record_type,expected_results,row)
...
def create_record_from_hazard_casue_link(product,component,record_type,expected_results,row){
	try{
	...
	String info='verify create_record_from_hazard_casue_link, on product: '+product+', record created by '+user_name+'\n'+'expected_results using spreadsheet: '+expected_results+'\n'
